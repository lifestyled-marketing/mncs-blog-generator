...
function buildUserPrompt({ topic, draft, messagingGuide, styleGuide }) {
  return [
    ...
    '5. Contains fabricated links. Specifically: any internal link that points to a slug on the brand\'s own website (for example `/blog/...`, `/services/...`, or any relative path) must be replaced with a placeholder in this exact format: `[INTERNAL LINK SUGGESTION: anchor text | topic: short description]` with NO URL. The writer does not know what pages exist on the site, so internal URLs are guesses. Strip the URL, keep the anchor text and a short topic description so a human can resolve it later. External Markdown links can stay as written; do not invent or "fix" their URLs.',
    '',
    'WHAT NOT TO DO:',
    ...
    '',
    '## REVISED DRAFT',
    'The complete blog post in clean Markdown, with inaccuracies corrected and everything else identical to the original. Keep all H1 / H2 / H3 headings, FAQ, [IMAGE: ...] placements, external Markdown links, and [INTERNAL LINK SUGGESTION: ...] placeholders from the original draft unless a specific item had to change for accuracy. If the writer left any fabricated internal URLs in the draft, convert them to [INTERNAL LINK SUGGESTION: anchor text | topic: short description] placeholders with no URL.',
    '',
